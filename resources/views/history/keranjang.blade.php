                    ...
                    <li class="list-none">
                        @auth
                            ...
                            <ul id="logoutMenu" class="hidden bg-white shadow-md rounded-md mt-2 p-2">
                                <li>
                                    <a href="#"
                                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-400">Logout</a>
                                </li>
                            </ul>
                        @endauth
                        @guest
            ...
            <div class="book-list shadow-sm grid">
                @forelse($cartItems as $index => $item)
                    <div class="book-item">
                        <div class="book-images">
                            ...
                            @foreach ($folders as $folder)
                                ...
                                @if (!$imageDisplayed && file_exists(public_path($imagePath)))
                                    <img src="{{ asset($imagePath) }}" alt="{{ $item['book_title'] }}"
                                        class="book-image" onerror="this.style.display='none'"
                        ...
                        <div>
                            <button class="borrow-button edit-btn mb-5"
                                onclick="openModal({{ $index }})">Edit</button>
                            <form action="{{ route('keranjang.delete', $index) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="borrow-button"
                                    style="background-color: #f44336;">Hapus</button>
                            </form>
                        </div>
                    </div>
                            ...
                            <form action="{{ route('keranjang.update', $index) }}" method="POST"
                                ...
                                <div>
                                    <label for="userName-{{ $index }}"
                                        class="block text-xl font-medium text-gray-700">Nama</label>
                                    <input type="text" id="userName-{{ $index }}" name="user_name"
                                        value="{{ $item['user_name'] }}"
                                        class="block w-full mt-1 border border-gray-300 shadow-md" required>
                                </div>

                                <div>
                                    <label for="email-{{ $index }}"
                                        class="block text-xl font-medium text-gray-700">Email</label>
                                    <input type="email" id="email-{{ $index }}" name="user_email"
                                        value="{{ $item['user_email'] }}"
                                        class="block w-full mt-1 border border-gray-300 shadow-md" required>
                                </div>

                                <div>
                                    <label for="borrowDate-{{ $index }}"
                                        class="block text-xl font-medium text-gray-700">Tanggal Pinjam</label>
                                    <input type="date" id="borrowDate-{{ $index }}" name="borrow_date"
                                        value="{{ $item['borrow_date'] }}"
                                        class="block w-full mt-1 border border-gray-300 shadow-md" required>
                                </div>

                                <div>
                                    <label for="returnDate-{{ $index }}"
                                        class="block text-xl font-medium text-gray-700">Tanggal Kembali</label>
                                    <input type="date" id="returnDate-{{ $index }}" name="return_date"
                                        value="{{ $item['return_date'] }}"
                                        class="block w-full mt-1 border border-gray-300 shadow-md" required>
                                </div>

                                <div class="flex justify-end space-x-2">
                                    <button type="button" onclick="closeModal({{ $index }})" class="btn"
                                        style="background-color: #f44336;">Batal</button>
